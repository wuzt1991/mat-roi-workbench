...
function legacyDB(dir,state){const store=new Store(dir);store.db.prepare('INSERT OR REPLACE INTO workspace VALUES(1,7,?,?)').run(JSON.stringify(state),'2026-09-01T00:00:00Z');store.close();}
...
test('旧数据库只迁移一次；更正和作废前原值保留，可取回完整旧备份',t=>{
  ...
  legacyDB(dir,old);store=new Store(dir);const result=store.read();assert.equal(result.revision,8);assert.equal(result.state.version,4);assert.equal(M.ledger(result.state).count,1);
...
test('坏旧账升级失败后，SQLite 原始数据与版本号不变',t=>{
  ...
  const recovering=new Store(dir);assert.throws(()=>recovering.read());assert.equal(recovering.recoveryStatus().canRestore,true);recovering.close();const db=new DatabaseSync(path.join(dir,'workbench.sqlite'));assert.equal(db.prepare('SELECT revision FROM workspace').get().revision,7);assert.deepEqual(JSON.parse(db.prepare('SELECT data FROM workspace').get().data),old);db.close();
});
test('账目更正、作废和删除计划可保存；删账、改成本和复活旧版本被拒绝',t=>{
  const dir=temp(),store=new Store(dir);cleanup(t,dir,()=>store.close());const s=seed(),a=entry(s);store.restore(s,0,'test-initial');
...
test('普通编辑每个日期最多一个恢复点；滚动保留 30 个，导入前保留 10 个，账目不裁剪',t=>{
  let date='2026-07-01T12:00:00Z';const dir=temp(),store=new Store(dir,{now:()=>date});cleanup(t,dir,()=>store.close());const s=seed();entry(s);let revision=store.restore(s,0,'test-initial').revision;
  ...
  assert.equal(store.backups().filter(x=>x.reason==='daily-v4').length,30);assert.equal(store.read().state.records.length,1);
  for(let i=0;i<15;i++)revision=store.restore(s,revision,'restore-'+i).revision;
  assert.equal(store.backups().filter(x=>x.reason==='before-restore-v4').length,10);assert.equal(store.read().state.records.length,1);
});
test('同范围更正链合并通过数据库保护，重复导入无重复账目',t=>{
  const dir=temp(),store=new Store(dir);cleanup(t,dir,()=>store.close());const local=seed(),a=entry(local);store.restore(local,0,'test-initial');
...
test('正式服务正确提供版本、字体、Excel 库；CSP 与无缓存生效',async t=>{
  const dir=temp(),running=createServer({dataDir:dir,port:0}),url=await running.listen();cleanup(t,dir,()=>running.close());
...
test('恢复不可借用保存队列；普通连续编辑只携带 save 语义',async()=>{
  ...
  assert.throws(()=>q.enqueue({value:1},'restore'),e=>e.code==='INVALID_SAVE_REASON');q.enqueue({value:1});const promise=q.flush();await Promise.resolve();q.enqueue({value:2});release();assert.equal(await promise,true);q.dispose();assert.deepEqual(writes.map(x=>x.reason),['save','save']);assert.equal(writes.at(-1).state.value,2);
});
