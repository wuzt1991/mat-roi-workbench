...
function appHarness(initial=M.initialState(),options={}){
  ...
  const window={MatModel:M,MatWorkbook:require('../public/workbook.js'),MatTransfer:require('../public/transfer.js'),MatTrends:require('../public/trends.js'),MatProductTransfer:require('../public/product-transfer.js'),ReusableRules:require('../public/reusable-rules.js'),PricingRules:require('../public/pricing-rules.js'),SalesImport:require('../public/sales-import.js'),PromotionRules:require('../public/promotion-rules.js'),addEventListener(){},confirm:()=>false,...options};
  const source=fs.readFileSync(path.join(__dirname,'../public/app.js'),'utf8').replace(/  initialize\(\);\r?\n\}\)\(\);/,`  window.testApp={set booted(v){booted=v;},get state(){return state;},set state(v){state=v;},get modal(){return modal;},set modal(v){modal=v;},set queue(v){queue=v;},transferRules,startEntry,saveModal,open,close,render,paintModal,skuTable,commonSizeChoices,skuOptions,listingCalculator,reusableForm,skuDisplayForm,productTransferPage,updateAction,updateStatusText,handleUpdateStatus};\n})();`);
