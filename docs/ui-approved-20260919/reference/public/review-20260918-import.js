(function(root){
  ...
  function getRules(state){return B.transferRules({...state,materials:state.materials.filter(m=>!m.deleted)});}
  ...
  function recalculateProducts(result,state){
    result.rules=getRules(state);delete result.thicknessPolicy;let blanks=0,pending=0,thicknessPending=0;
