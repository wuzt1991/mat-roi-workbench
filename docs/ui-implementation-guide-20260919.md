...
接入结果修订（2026-09-21）：UI-U01 已落实为正式右上角主题按钮旁的 `#ui-motion-toggle`，不是预览工具栏。按钮调用唯一 `UiAppearance.setMotion`，状态保存到 `mat-workbench-ui-study-motion-v1`；根属性关闭 CSS/JS 动效，系统减少动态依然生效。按钮可随 #app 重绘重建，但事件只绑定一次。两主题、刷新偏好和窄屏在当前内测浏览器验证；最终结果见 [交付记录](release-1.2.2-internal.md)。下文入口未定等描述为冻结交接时的历史状态。
