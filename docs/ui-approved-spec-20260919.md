...
正式接入修订（2026-09-21）：1.2.2 内测保留本视觉方案，UI-U01 已补齐：右上 `.topbar-right` 中主题按钮旁加入唯一 `#ui-motion-toggle`，显示“动效：开/关”，有 aria-pressed 和键盘支持，沿用现有偏好键与 UiAppearance.setMotion。系统减少动态优先禁止动画；不带入预览工具栏。计划入口采用用户后续确认的经典列表；销售导入采用当前尺寸汇总弹窗与文件拖入。冻结资源保留作视觉来源，正式业务代码以当前实现为准。
