...
async function upgrade(){
 ...
 const checks=[],errors=[],network=[],versions=new Map(),payloads=[previous,candidate].map(executable=>({executable,sha256:hash(path.resolve(path.dirname(executable),'../Resources/app.asar'))}));
 ...
 async function start(executable){
  ...
  const end=Date.now()+45000;while(true){try{const health=await(await fetch(url+'/api/health')).json();assert.equal(health.dataDir,dir);versions.set(executable,health.version);break;}catch(e){if(Date.now()>end)throw e;await new Promise(r=>setTimeout(r,200));}}
 ...
 try{
  ...
  const oldSaved=await read();assert.deepEqual(oldSaved.state,seed);await stop();checks.push(`${versions.get(previous)} 在隔离目录创建数据库和冻结账目`);
  ...
  const restored=await read();assert.deepEqual(restored.state,next);assert.ok(restored.storageEpoch>before.storageEpoch);await stop();checks.push(`${versions.get(candidate)} 读取旧库、保存、导出 Excel、替换恢复并推进存储代次`);
  await start(previous);const rolled=await read();assert.deepEqual(rolled.state,next);assert.deepEqual(rolled.state.records,seed.records);await stop();checks.push(`实际 ${versions.get(previous)} 程序回读候选最新数据库，冻结账目与所有业务字段一致`);
