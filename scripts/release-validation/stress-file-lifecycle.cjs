...
async function worker(){
 while(next<runs){
  ...
  const result=await new Promise(resolve=>{
   let output='';const child=spawn(process.execPath,['--test',path.resolve(__dirname,'../../test/file-service-lifecycle.test.cjs')]);
...
Promise.all(Array.from({length:concurrency},worker)).then(()=>{
 const report={host:process.platform,node:process.version,root:process.env.MAT_VERIFY_ROOT||path.resolve(__dirname,'../..'),runs:results.length,concurrency,failed:results.filter(r=>r.code!==0).length,results};
 report.passed=report.failed===0&&report.runs===runs;
