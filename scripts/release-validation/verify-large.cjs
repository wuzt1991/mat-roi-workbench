...
async function verify(){
  ...
  report.expectedRowHash=expected.digest('hex');report.pageP95Ms=latencies.sort((a,b)=>a-b)[Math.ceil(latencies.length*.95)-1];assert.ok(report.pageP95Ms<=500,'Deep page p95 exceeds 500ms');
  // Exercise edits against the full group, not just read-only paging.
  const first=store.page().rows[0],other=rules.materials.find(m=>m.name==='亚麻');
  const edit={mutationId:'large-single-edit',ownerToken:'validation',expectedSessionRevision:store.getMeta('revision'),rowIds:[first.rowId],action:{type:'row-edit'},patch:{material:{mode:'value',materialId:other.id}}};
  let editStarted=performance.now();const receipt=store.applyReview(edit,rules);report.singleEditMs=performance.now()-editStarted;assert.equal(receipt.changed,1);assert.equal(store.applyReview(edit,rules).replayed,true);
  editStarted=performance.now();store.undo({ownerToken:'validation',expectedSessionRevision:store.getMeta('revision')},rules);report.singleUndoMs=performance.now()-editStarted;assert.ok(report.singleEditMs<=500,'One-row edit exceeds 500ms');assert.ok(report.singleUndoMs<=500,'One-row undo exceeds 500ms');
  const bulk={mutationId:'large-group-edit',ownerToken:'validation',expectedSessionRevision:store.getMeta('revision'),groupId:first.groupId,action:{type:'group-unify'},patch:{material:{mode:'value',materialId:other.id}}};
  editStarted=performance.now();const all=store.applyReview(bulk,rules);report.groupEditMs=performance.now()-editStarted;assert.equal(all.changed,count);assert.equal(store.page().groups[0].materialState,other.id);
  editStarted=performance.now();assert.equal(store.undo({ownerToken:'validation',expectedSessionRevision:store.getMeta('revision')},rules).undone,count);report.groupUndoMs=performance.now()-editStarted;assert.equal(store.counts().ready,true);store.close();
  ...
  report.maxRssMiB=process.resourceUsage().maxRSS/1024;report.totalMs=Date.now()-started;report.exceeds512MiBRssTarget=report.maxRssMiB>512;assert.ok(!report.exceeds512MiBRssTarget,'Validation process exceeds the 512 MiB memory budget');report.passed=true;persist();console.log(JSON.stringify(report));
}
