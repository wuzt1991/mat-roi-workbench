...
(() => {
  ...
  let filters={shopId:'',planId:'',from:H.shift(M.today(),-6),to:M.today(),includeOld:false};
  let operatingFilters={from:H.shift(M.today(),-6),to:M.today(),unit:'day',includeOld:false},operatingMetric='spend',operatingPage=0,ledgerGroup='plan',ledgerSort='profit-desc';
  ...
  const dimensions=s=>{const d=M.productionDimensions(s);return s?`${d.width||'—'} × ${d.height||'—'} cm`:'';};
  ...
  function nav(){return `<aside class="sidebar">
    ...
    <section class="plan-nav-section" aria-label="我的计划"><div class="nav-title">${btn('plans-view','我的计划','','ghost')}${ib('new-plan','新建计划','plus')}</div><div class="plan-list-scroll">${activePlans().map((p,i)=>{const r=M.calculate(state,p);return `<div class="plan-nav-row ${state.active===p.id?'active':''}"><button type="button" class="plan-nav" data-action="select-plan" data-id="${e(p.id)}" ${state.active===p.id?'aria-current="true"':''}><span class="plan-symbol">${i+1}</span><span class="grow"><span class="plan-name" title="${e(p.name)}">${e(p.name)}</span><span class="plan-roi" data-sidebar="${e(p.id)}">保本 ROI ${r.valid?roi(r.roi):'待完善'}</span></span></button><div class="plan-nav-tools">${ib('pin-plan',(p.pinned?'取消置顶 ':'置顶 ')+p.name,p.pinned?'pin-off':'pin',`data-id="${e(p.id)}" aria-pressed="${!!p.pinned}"`)}${ib('delete-plan','删除 '+p.name,'x',`data-id="${e(p.id)}"`)}</div></div>`;}).join('')||'<p class="note sidebar-empty">暂无计划</p>'}</div></section>
  ...
  function historyPage(all){const f=all?filters:{...operatingFilters,shopId:state.activeShop,planId:current().id};return `${all?'<div class="page-header"><div class="page-heading"><h1>总账</h1><p class="page-sub">汇总有效经营记录，比较店铺与计划表现。</p></div>'+btn('export-ledger','导出筛选账目','file-spreadsheet')+'</div><div class="wide-content">':''}${O.page(state,f,{all,group:ledgerGroup,sort:ledgerSort,page:operatingPage,metric:operatingMetric})}${all?'</div>':''}`;}
  ...
  function sizeList(){const list=state.sizes.filter(s=>(showDeletedRules||!s.deleted)&&(sizeName(s)+' '+dimensions(s)).includes(query));return list.length?`<div class="table-scroll"><table class="size-table"><thead><tr>${['商品规格','实际生产尺寸','生产面积 / ㎡','状态','操作'].map(x=>`<th scope="col">${x}</th>`).join('')}</tr></thead><tbody>${list.map(s=>`<tr><td><strong>${e(sizeName(s))}</strong></td><td>${e(production(s))}</td><td>${n(M.productionArea(s),4)}</td><td><span class="tag ${s.needsReview?'amber':''}">${s.needsReview?'待补生产尺寸':s.active?'启用':'停用'}</span></td><td>${btn('edit-size','编辑','','ghost',`data-id="${s.id}" aria-label="编辑 ${e(sizeName(s))}"`)}${s.deleted?btn('restore-rule','恢复','rotate-ccw','ghost',`data-kind="sizes" data-id="${s.id}"`):btn('toggle-size',s.active?'停用':'启用','','ghost',`data-id="${s.id}"`)+ib('delete-rule','删除 '+sizeName(s),'trash-2',`data-kind="sizes" data-id="${s.id}"`)}</td></tr>`).join('')}</tbody></table></div>`:empty('没有匹配的规格','可以新增公共规格供所有计划使用。','new-size','新增规格');}
  ...
  function sizeChoices(ids=[]){return state.sizes.filter(s=>s.active||ids.includes(s.id)).map(s=>`<label class="select-size"><input type="checkbox" data-select-size="${s.id}" ${ids.includes(s.id)?'checked':''}><span class="size-mark"></span><div><strong>${e(sizeName(s))}</strong><div class="note">实际生产 ${e(production(s))}</div></div></label>`).join('');}
  function sizeForm(){const d=modal.draft;frame(modal.id?'编辑规格':'新增规格',`<p class="dialog-note">填写实际生产尺寸，用于计算面积、重量和成本。历史账目保持原值。</p><div class="size-form-layout"><div class="size-fields">${field('规格名称（可选）','name',d.name,'text','maxlength="80"')}<div class="form-group-title">实际生产尺寸</div><div class="dialog-grid">${field('生产长 / cm','salesW',d.salesW)}${field('生产宽 / cm','salesH',d.salesH)}</div></div><aside class="size-preview"><div class="shape-illustration"><span></span></div><span class="note">生产面积</span><strong id="area-preview">${n(M.productionArea({...d,needsReview:false}),4)} ㎡</strong><p class="note">材料成本 = 生产面积 × 材料单价</p></aside></div>`);}
  ...
  function validateEntry(){
    ...
    for(const key of ['spend',p.params.revenueInput==='amount'?'actualGmv':'actualRoi','fee','tax','recovery','other','returnCost'])if(!M.nonnegative(p.params[key])||['fee','tax','recovery'].includes(key)&&p.params[key]>100){fail('请填写有效的费用和比例。',`[data-entry-param="${key}"]`);return false;}
  ...
  function startEntry(record,date=M.today()){const p=current();if(!record&&!p)return;if(!record){const existing=state.records.find(h=>h.kind==='daily'&&h.status==='confirmed'&&h.planId===p.id&&h.date===date);if(existing)return open('record-detail',{id:existing.id});}const frame=record?M.clone(record.frame):M.makeFrame(state,p);if(!frame)return toast('这笔旧记录缺少完整成本，无法直接更正');if(!record){const result=M.calculate(frame,frame.plan);frame.plan.params.revenueInput='amount';frame.plan.params.actualGmv=result.gmv??'';}open('entry',{frame,date:record?.date||date,note:record?.note||'',reason:'',previousId:record?.id||null});}
  ...
  function saveModalCandidate(){const d=modal.draft,p=current(),kind=modal.type;
    ...
    if(kind==='size'){
      const value={id:modal.id||M.uid('size'),name:d.name.trim(),salesW:d.salesW,salesH:d.salesH,irregular:false,productionW:'',productionH:'',active:d.active??true,deleted:d.deleted??false,needsReview:false};
      if(!M.validSize(value)){const key=['salesW','salesH'].find(key=>!M.positive(value[key])||value[key]>10000)||'name';return fail('请填写大于 0、不超过 10000 cm 的实际生产长宽',`[data-field="${key}"]`);}
  ...
  document.addEventListener('click',async event=>{
    ...
    try{
      ...
      if(a==='operating-range'){const f=view==='history'?filters:operatingFilters;f.from=H.shift(M.today(),1-Number(b.dataset.days));f.to=M.today();operatingPage=0;render();return;}
      if(a==='operating-all'){const f=view==='history'?filters:operatingFilters;f.from='';f.to='';operatingPage=0;render();return;}
      if(a==='operating-page'){operatingPage=Math.max(0,Number(b.dataset.page)||0);render();return;}
      if(a==='operating-group'){ledgerGroup=b.dataset.group;operatingPage=0;render();return;}
      if(a==='operating-shop'){filters.shopId=id;filters.planId='';ledgerGroup='plan';operatingPage=0;render();return;}
      if(a==='operating-plan'){const target=state.plans.find(p=>p.id===id);if(target&&!target.deleted&&!state.shops.find(s=>s.id===target.shopId)?.deleted){state.active=target.id;state.activeShop=target.shopId;operatingFilters={...operatingFilters,from:filters.from,to:filters.to,includeOld:false};setView('plan');tab='history';operatingPage=0;commit();}else{filters.planId=id;ledgerGroup='records';operatingPage=0;render();}return;}
      ...
      if(a==='export-ledger'){if(filters.from&&filters.to&&filters.from>filters.to)return toast('开始日期不能晚于结束日期');return startExport(view==='history'?filters:{...operatingFilters,shopId:state.activeShop,planId:p?.id});}
      ...
      if(a==='tab'){tab=b.dataset.tab;operatingPage=0;render();return;}if(a==='sku-settings')return open('sku-settings',{id,draft:M.clone(p.items.find(x=>x.id===id))});
...
if(a==='sku-options'){skuOptionsOpen=!skuOptionsOpen;render();return;}if(a==='restore-price'){commit(R.resumePrice(state,p.id,id));return;}
      ...
      if(a==='confirm-record'){
        ...
        const entryPlanId=modal.frame.plan.id,entryDate=modal.date;if(view==='plan'&&current()?.id===entryPlanId){tab='history';if(operatingFilters.from&&entryDate<operatingFilters.from||operatingFilters.to&&entryDate>operatingFilters.to){operatingFilters.from=H.shift(entryDate,-6);operatingFilters.to=entryDate;}operatingPage=0;}await persistModal(candidate,'save','已确认入账，历史成本已锁定');return;
      }
  ...
  document.addEventListener('change',event=>{
    ...
    if(el.matches('[data-operating]')){operatingFilters[el.dataset.operating]=value;operatingPage=0;render();return;}
    if(el.matches('[data-operating-metric]')){operatingMetric=value;render();return;}
    if(el.matches('[data-operating-sort]')){ledgerSort=value;render();return;}
    if(el.matches('[data-filter]')){filters[el.dataset.filter]=value;operatingPage=0;if(el.dataset.filter==='shopId')filters.planId='';render();}
