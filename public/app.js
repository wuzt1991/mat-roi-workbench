...
(() => {
  'use strict';
  const M=window.MatModel,W=window.MatWorkbook,T=window.MatTransfer,H=window.MatTrends,P=window.MatProductTransfer;
  ...
  const materialOptions=id=>state.materials.filter(m=>((m.active&&(m.weightRules||[]).some(materialRuleUsable))||m.id===id)).map(m=>option(m.id,`${m.name}${m.active?'':'（停用）'}`,id)).join('');
  ...
  const shippingOptions=id=>state.shippingTemplates.filter(t=>t.active||t.id===id).map(t=>option(t.id,t.name+(t.active?'':'（停用）'),id)).join('');
  ...
  function normalize(){
    if(!state.shops.some(s=>s.id===state.activeShop&&!s.deleted))state.activeShop=state.shops.find(s=>!s.deleted)?.id||state.shops[0].id;
  ...
  function updateInstallBlocked(){return !booted||!!modal||!!queue?.pending||!!queue?.sending||!!saveError||saveStatus!=='saved';}
  ...
  async function initialize(){
    ...
    try{
      const saved=await request('/api/state');let revision=saved.revision,migrationChanged=false;
      state=M.migrate(saved.state||M.initialState());
      migrationChanged=!!saved.state&&JSON.stringify(saved.state)!==JSON.stringify(state);
      if(!M.validateBackup(state))throw Error('数据未通过检查，原数据库已保留。');
      if(!saved.state){const initial=await request('/api/state',{method:'PUT',body:JSON.stringify({state,revision,reason:'migration'})});revision=initial.revision;}
      normalize();queue?.dispose();
      fetch('assets/product-template.xlsx').then(r=>r.ok?r.arrayBuffer():Promise.reject(Error('内置商品模板读取失败'))).then(arrayBuffer=>{productFiles.template={name:'内置商品模板（固定）',arrayBuffer};render();}).catch(error=>toast(safeError(error,'文件读取失败，请重新选择有效的 Excel 文件后重试。')));
      queue=new window.WorkbenchPersistence.SaveQueue({revision,
        send:(data,rev,reason)=>request('/api/state',{method:'PUT',body:JSON.stringify({state:data,revision:rev,reason})}),
        cache:{write:data=>localStorage.setItem(draftKey,JSON.stringify(data)),clear:()=>localStorage.removeItem(draftKey)},
        status:(status,error)=>{saveStatus=status;saveError=error?safeError(error,error.status===409?'其他窗口已更新，请导出本窗口草稿后重新载入。':'保存失败，请检查磁盘空间后重试；关闭前请导出 Excel 备份。'):'';paintSaveStatus();}
      });
      const regional=M.regionalShippingTemplate?.();
      if(regional){let changed=false;if(!state.shippingTemplates.some(t=>t.type==='regional')){state.shippingTemplates.push(regional);changed=true;}for(const plan of state.plans){const currentTemplate=state.shippingTemplates.find(t=>t.id===plan.shippingId);if(currentTemplate?.type==='fixed'&&currentTemplate.fee===1.35){plan.shippingId=regional.id;changed=true;}if(plan.params.destination!==undefined){delete plan.params.destination;changed=true;}}migrationChanged=migrationChanged||changed;}
      if(migrationChanged)save('migration');
      saveStatus=migrationChanged?'pending':'saved';saveError='';booted=true;render();
      const drafts=[];
      try{for(const key of Object.keys(localStorage).filter(k=>k.startsWith(DRAFT_PREFIX))){try{const draft=JSON.parse(localStorage.getItem(key));if(!draft?.state)continue;draft.state=M.migrate(draft.state);if(JSON.stringify(draft.state)===JSON.stringify(state)){localStorage.removeItem(key);continue;}drafts.push({key,...draft});}catch{}}}catch{}
      pendingRecovery=drafts.sort((a,b)=>b.updated-a.updated)[0]||null;
      if(pendingRecovery)open('recover-draft');
    }catch(error){saveStatus='error';saveError=safeError(error,'数据读取失败，请重试或从恢复点导出备份后联系维护人员。');app.innerHTML=`<main class="startup-error"><h1>数据暂时无法打开</h1><p role="alert">${e(saveError)}</p><p>原数据已保留，请重试或下载恢复点。</p>${btn('boot-retry','重新读取','refresh-cw','primary')}${btn('recovery-points','查看恢复点','history')}</main>`;}
  }
  ...
  async function downloadState(data,name){const migrated=M.migrate(data);if(!M.validateBackup(migrated))throw Error('恢复点未通过校验，请保留数据库并联系维护人员');const bytes=await W.exportWorkbook(migrated),url=URL.createObjectURL(new Blob([bytes],{type:'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'})),link=document.createElement('a');link.href=url;link.download=name+'.xlsx';link.click();setTimeout(()=>URL.revokeObjectURL(url),30000);}
  ...
  function nav(){return `<aside class="sidebar">
    ...
    <nav class="nav-group" aria-label="主导航">${[['plan-view','投放测算','layout-dashboard','plan'],['ledger-view','总账','chart-no-axes-combined','history'],['library-view','公共资料','library','library'],['product-view','商品转表','table-2','product']].map(([a,l,i,v])=>btn(a,l,i,'nav-item '+(view===v?'active':''),view===v?'aria-current="page"':'')).join('')}</nav>
    <div class="plan-nav-section"><div class="nav-title">${btn('plans-view','我的计划','','ghost '+(view==='plans'?'active':''),view==='plans'?'aria-current="page"':'')}${ib('new-plan','新建计划','plus')}</div><div class="plan-list-scroll">${activePlans().map((p,i)=>{const r=M.calculate(state,p),selected=current()?.id===p.id&&view==='plan';return `<button type="button" class="plan-nav ${selected?'active':''}" ${selected?'aria-current="page"':''} data-action="select-plan" data-id="${p.id}"><span class="plan-symbol">${i+1}</span><div class="grow"><div class="plan-name" title="${e(p.name)}">${e(p.name)}</div><div class="plan-roi" data-sidebar="${p.id}">保本 ROI ${r.valid?roi(r.roi):'待完善'}</div></div></button>`;}).join('')||'<p class="note sidebar-empty">这个店铺还没有计划</p>'}</div></div>
  ...
  function planPage(){const p=current();if(!p)return empty('这个店铺还没有计划','从已有材料和尺寸开始，填写一次即可反复测算。');const r=M.calculate(state,p),t=state.shippingTemplates.find(t=>t.id===p.shippingId),pm=state.materials.find(m=>m.id===p.materialId),pr=M.materialRule(pm,p);return `<div class="page-header"><div class="page-heading"><div class="row"><h1>${e(p.name)}</h1>${ib('rename-plan','重命名计划','pencil')}</div><p class="page-sub">修改参数即时测算，确认入账后计入总账。</p></div><div class="row">${btn('copy-plan','复制计划','copy')}${btn('entry','确认入账','book-check','confirm-entry')}</div></div>
    ...
    ${tab==='sku'?`<div class="material-strip"><div class="plan-resource"><label class="field"><span>本计划材料</span><select data-plan-material aria-label="本计划材料">${materialOptions(p.materialId)}</select></label><label class="field"><span>厚度</span><select data-plan-material-rule aria-label="本计划材料厚度">${(pm?.weightRules||[]).filter(materialRuleUsable).map(r=>option(r.id,`${r.thickness===undefined||r.thickness===''?'不限厚度':r.thickness+' mm'}`,p.materialRuleId)).join('')}</select></label>${btn('edit-current-material','编辑材料与规则','pencil','ghost')}<p class="note">${materialRuleUsable(pr)?`材料成本 ¥${Number(pr.costPerSqm).toFixed(2)} / ㎡ · 重量系数 ${pr.coefficient}`:'请先在材料库添加有效厚度规则'}</p></div><div class="plan-resource"><label class="field"><span>运费模板</span><select data-plan-shipping aria-label="运费模板">${shippingOptions(p.shippingId)}</select></label>${ib('shipping-library','管理运费模板','settings-2')}<label class="field"><span>固定包装重量 / g</span><input type="number" data-packaging-weight data-unit="g" value="${e(M.toGrams(p.packagingWeight??0))}" min="0" max="1000000" step="any" aria-label="固定包装重量 / g"></label><p class="note">${e(ruleText(t))} · 自动重量含已填包装重量 · 普通省份最高常规价 · 偏远四省人工核价</p></div></div>`:''}
  ...
  function skuCell(id,i){
    if(id==='price'||id==='share')return `<td><input type="number" data-item="${i.sizeId}" data-key="${id}" value="${e(i[id])}" step="any" min="0" ${id==='share'?'max="100"':''} aria-label="${e(sizeName(i.size)+' '+(id==='price'?'售价':'订单占比'))}"></td>`;
    if(id==='weight')return `<td class="num"><button class="sku-weight" data-action="weight" data-id="${i.sizeId}" aria-label="编辑 ${e(sizeName(i.size))} 发货重量">${i.weight===''?'填写重量':e(weightText(i.weight))}</button></td>`;
    const value=id==='roi'?(!M.positive(i.price)||!Number.isFinite(i.cost)?'待完善':roi(i.roi)):money(i[id]);
    return `<td class="num" data-row-${id}="${i.sizeId}" ${id==='cost'?`title="${e('运费 '+money(i.shipping)+'；含退货影响及已填费用，未含广告')}"`:''}>${value}</td>`;
  }
  function skuTable(r){
    const columns=M.skuColumns(state),heads=columns.map(id=>M.skuColumnList.find(c=>c.id===id));
    return `<div class="section-head"><div class="row"><h2>商品规格</h2><span class="tag">${r.rows.length} 个</span></div><div class="row">${btn('sku-display','显示设置','sliders-horizontal','ghost','aria-label="商品规格显示设置"')}${btn('add-skus','添加规格','plus')}</div></div>${r.rows.length?`<div class="table-scroll"><table class="sku-table" style="--sku-min:${260+columns.length*98}px"><caption class="sr-only">商品规格及每单成本</caption><thead><tr><th scope="col">商品规格</th>${heads.map(c=>`<th scope="col" data-column="${c.id}" ${c.id==='cost'?'title="含材料、运费、平台费、税及已填费用，计入退货影响，未含广告"':''}>${c.label}${c.unit?`<small>/ ${c.unit}</small>`:c.id==='cost'?'<small>未含广告</small>':''}</th>`).join('')}<th scope="col"><span class="sr-only">操作</span></th></tr></thead><tbody>${r.rows.map(i=>`<tr><td><div class="sku-title">${e(sizeName(i.size))}${i.size?.irregular?'<span class="tag">异形</span>':''}</div><div class="sku-sub">${i.size?.irregular?`销售 ${e(dimensions(i.size))}<br>`:''}生产 ${e(production(i.size))}</div></td>${columns.map(id=>skuCell(id,i)).join('')}<td>${ib('weight','编辑 '+sizeName(i.size)+' 含包装重量','weight',`data-id="${i.sizeId}"`)}${ib('remove-sku','移除 '+sizeName(i.size),'x',`data-id="${i.sizeId}"`)}</td></tr>`).join('')}</tbody></table></div><div class="total-row"><span>各规格按生产尺寸计成本，按含包装重量计运费。</span><div class="row"><span id="share-total">合计 ${n(r.total,0)}%</span>${ib('normalize-share','调整占比至 100%','equal')}</div></div>`:empty('还没有商品规格','从公共尺寸库选择后填写售价和占比。','add-skus','添加规格')}<div id="validation">${errors(r)}</div>`;
  }
  ...
  function productTransferPage(){
    ...
    return `<div class="page-header"><div class="page-heading"><h1>商品转表</h1><p class="page-sub">上传 ERP 商品规格，使用固定模板识别字段后生成静态值商品表。</p></div><div class="row">${btn('product-export','导出商品表','download','primary',(!r||!r.summary?.ready||productBusy)?'disabled':'')}</div></div>
      <div class="wide-content product-transfer-page">
        ...
        <section class="product-rules"><div class="section-head"><div><h2>材料规则来源</h2><p class="note">商品转表直接读取公共资料中的材质、厚度、重量系数和成本价。请在材料库维护唯一规则。</p></div>${btn('library-view','打开材料库','library','ghost')}</div></section>
  ...
  function render(){const focus=focusSnapshot(app);normalize();app.innerHTML=`<div class="app-shell">${nav()}<main class="workspace" id="main-content" tabindex="-1">${topbar()}${({plan:planPage,plans:plansPage,shops:shopsPage,library:libraryPage,history:()=>historyPage(true),data:dataPage,product:productTransferPage}[view]||planPage)()}</main></div>`;window.lucide?.createIcons();paintSaveStatus();decorateTables(app);restoreFocus(app,focus);markProductErrors();}
  function refreshCalculation(){const p=current();if(!p)return;const r=M.calculate(state,p),el=document.querySelector('#metrics');if(el)el.innerHTML=metrics(r);for(const i of r.rows){for(const [selector,value] of [['material',money(i.material)],['shipping',money(i.shipping)],['cost',money(i.cost)],['roi',!M.positive(i.price)||!Number.isFinite(i.cost)?'待完善':roi(i.roi)]]){const cell=document.querySelector(`[data-row-${selector}="${i.sizeId}"]`);if(cell)cell.textContent=value;}}for(const i of r.rows){const cell=document.querySelector(`[data-row-cost="${i.sizeId}"]`);if(cell)cell.title='按退款类型分摊商品和运费；含平台费、税及其他费用，未含广告';}const total=document.querySelector('#share-total');if(total)total.textContent=`合计 ${n(r.total,2)}%`;const breakdown=document.querySelector('#refund-breakdown');if(breakdown)breakdown.textContent=refundSummaryText(p.params);const error=document.querySelector('#validation');if(error)error.innerHTML=errors(r);const side=document.querySelector(`[data-sidebar="${p.id}"]`);if(side)side.textContent='保本 ROI '+(r.valid?roi(r.roi):'待完善');window.lucide?.createIcons();}
  ...
  function close(force=false){if(!modal)return;if(!force&&modal.pending)return;if(!force&&!modal.committed&&modal.baseline!==modalValue()&&!window.confirm('有未保存的修改，确定放弃并关闭吗？'))return;const parent=modal?.parent;dialog.close();modal=null;if(parent){modal=parent;paintModal();dialog.showModal();}else if(lastFocus?.isConnected)lastFocus.focus();else document.querySelector('#main-content')?.focus();}
  ...
  function entryForm(){const f=modal.frame,p=f.plan,m=f.materials[0],r=M.calculate(f,p),quotes=state.materials.find(x=>x.id===m.id)?.history||m.history||[];
    frame(modal.previousId?'更正已入账记录':'确认当天入账',`<div class="entry-context"><span>${e(shopName(p.shopId))}</span>${icon('chevron-right')}<strong>${e(state.plans.find(x=>x.id===p.id)?.name)}</strong><span class="tag">${modal.previousId?'保留原记录':'核对后才计入总账'}</span></div><p class="dialog-note">${modal.previousId?'以下从原记录的成本和费用开始，更改只影响这笔账。':'以下是本次入账副本。请填当天实际数据，材料价可选旧报价或实际平均价。'}</p><div class="entry-columns"><section><div class="dialog-grid">${field('入账日期','date',modal.date,'date',`max="${M.today()}"`)}${field('入账备注','note',modal.note,'text','maxlength="100"')}</div><h3 class="form-group-title">当天投放与费用</h3><div class="dialog-grid">${paramField('spend','广告消耗','元',p.params,true)}${paramField('actualRoi','支付 ROI','',p.params,true)}${entryRefundFields(p.params)}<label class="field"><span class="field-label">其他费用发生范围</span><select data-entry-param="otherFeeScope" aria-label="入账其他费用发生范围">${option('shipped','已发货订单',p.params.otherFeeScope)}${option('all','所有订单',p.params.otherFeeScope)}</select></label>${paramField('fee','平台服务费','%',p.params,true)}${paramField('tax','税率','%',p.params,true)}${paramField('recovery','退货回收比例','%',p.params,true)}${paramField('other','其他费用','元/单',p.params,true)}${paramField('returnCost','每退货单额外费用','元',p.params,true)}</div></section><aside class="entry-cost-panel"><h3>本次用料成本</h3><p>${e(m.name)}</p><label class="field"><span>参考报价</span><select data-quote aria-label="选择历史材料报价"><option value="">选择报价或直接填单价</option>${quotes.slice().reverse().map(h=>`<option value="${h.price}">${e(h.date)} · ${money(h.price)}/㎡</option>`).join('')}</select></label><label class="field"><span>本次材料单价 / ㎡</span><input type="number" data-entry-price aria-label="本次材料单价" value="${e(M.frameMaterialPrice(f))}" min="0" step="any"></label><p class="note">混用不同批次时，按实际耗用面积算平均价，不按调价日期自动切分。</p>${entryShipping()}<div id="entry-summary">${entrySummary(r)}</div></aside></div><details class="entry-items" ${modal.previousId?'open':''}><summary>核对当天规格、售价、占比和重量</summary><div class="table-scroll"><table><thead><tr><th scope="col">规格 / 生产尺寸</th><th scope="col">售价 / 元</th><th scope="col">订单占比 / %</th><th scope="col">重量 / g</th></tr></thead><tbody>${p.items.map(i=>{const s=f.sizes.find(s=>s.id===i.sizeId);return `<tr><td>${e(sizeName(s))}<div class="note">${e(production(s))}</div></td>${['price','share','weight'].map(k=>`<td><input type="number" data-entry-item="${i.sizeId}" data-key="${k}" ${k==='weight'?'data-unit="g"':''} aria-label="入账 ${e(sizeName(s))} ${{price:'售价',share:'占比',weight:'重量 / g'}[k]}" value="${e(k==='weight'?M.toGrams(i[k]):i[k])}" step="any"></td>`).join('')}</tr>`;}).join('')}</tbody></table></div></details>${modal.previousId?field('更正原因（必填）','reason',modal.reason,'text','maxlength="200"'):''}<p class="note stack-gap">入账按退款类型分摊商品和运费，含平台费、税及其他费用，不含广告。之后有实际数据可更正，历史成本不会跟随今天的报价变化。</p>`,btn('close','继续核对')+btn('confirm-record',modal.previousId?'确认更正并入账':'确认入账','book-check','confirm-entry'));}
  ...
  function paintModal(){
    ...
    if(modal.type==='trend-demo')return trendDemo();
  ...
  function validateEntry(){
    ...
    if(!M.nonnegative(M.frameMaterialPrice(f))){fail('请填写有效的非负材料单价。','[data-entry-price]');return false;}
    ...
    for(const item of p.items)for(const key of ['price','share','weight'])if(key==='price'&&!M.positive(item[key])||key==='share'&&(!M.nonnegative(item[key])||item[key]>100||Math.abs(r.total-100)>1e-6)||key==='weight'&&item[key]!==''&&(!M.positive(item[key])||M.shippingCost(f.shippingTemplates[0],item[key]).error)){fail('请核对售价、占比和模板支持的含包装重量；占比合计需为 100%。',`[data-entry-item="${item.sizeId}"][data-key="${key}"]`);return false;}
  ...
  function restoreForm(){
    ...
    frame('导入 Excel',`<div class="import-source"><strong>${scope?'按范围导出':'完整工作区'} · ${data.shops.length} 家店铺 · ${data.plans.length} 个计划</strong><p>${e(data.shops.map(s=>s.name).join('、'))}</p><p>${scope?`${scope.from||'不限'} 至 ${scope.to||'不限'}`:'全部日期'} · ${data.records.length} 个账目版本</p></div>${scope?'':`<div class="export-modes"><label><input type="radio" name="restore-mode" data-restore-mode value="merge" ${!replace?'checked':''}>合并到本机</label><label><input type="radio" name="restore-mode" data-restore-mode value="replace" ${replace?'checked':''}>替换整个工作区</label></div>`}${replace?'<p class="message-error">将替换本机全部店铺、计划、公共资料和账目。请先导出当前工作区留存。</p>':`<label class="restore-plan-choice"><input type="checkbox" data-restore-plans ${modal.restorePlans?'checked':''}>同时恢复所选计划的试算参数</label><p class="note">${modal.restorePlans?'按文件恢复所选计划的材料、运费规则、售价和占比；其他计划保持原样。':'已有计划保留本机试算；新计划始终按文件恢复。'}</p><div class="export-preview"><strong>新增 ${r.shops} 家店铺 · ${r.plans} 个计划 · ${r.added} 笔账目</strong><p>更新 ${r.updated} 个账目状态 · ${r.duplicates} 笔重复记录不再导入</p><p>补充 ${r.resources} 份公共资料${modal.restorePlans?` · 更新 ${r.plansUpdated} 个计划试算`:''}。其他计划使用的公共资料和本机显示设置保持原样。</p></div><p class="note stack-gap">仅合并文件中的数据。其他店铺和范围外账目保留；关联的更正记录会一起更新。</p>${r.conflicts.length?`<div class="message-error import-conflicts"><strong>${r.conflicts.length} 个计划有冲突：这些计划的账目全部保留本机版本，不导入文件版本。</strong><ul>${r.conflicts.map(c=>`<li>${e(c.name)}：${e(c.reason)}</li>`).join('')}</ul></div>`:''}`}`,btn('export','先导出本机 Excel','download')+btn('close','取消')+btn('confirm-restore',replace?'替换并恢复':r.conflicts.length?'合并无冲突数据':'确认合并恢复','check',replace?'danger':'primary'));
  }
  async function exportExcel(){if(busy)return;busy=true;toast('正在生成 Excel…');try{const selected=T.select(state,modal.scope),bytes=await W.exportWorkbook(selected),blob=new Blob([bytes],{type:'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'}),url=URL.createObjectURL(blob),a=document.createElement('a');a.href=url;a.download=`地垫工作台-${selected.exportScope?'所选范围':'完整备份'}-${M.today()}-${new Date().toTimeString().slice(0,8).replaceAll(':','')}.xlsx`;a.click();setTimeout(()=>URL.revokeObjectURL(url),30000);if(modal?.type==='export')close(true);toast('Excel 已生成，请保存到电脑');}catch(error){toast(safeError(error,'文件处理失败，请检查文件格式和数据后重试。'));}finally{busy=false;}}
  function saveModal(){const d=modal.draft,p=current(),kind=modal.type;
    if(kind==='display'){state.prefs.ids=[...modal.ids];commit();close(true);return;}
    if(kind==='sku-display'){state.prefs.skuColumns=[...modal.ids];commit();close(true);return;}
    if(kind==='add-skus'){p.items=p.items.filter(i=>modal.ids.includes(i.sizeId));for(const id of modal.ids)if(!p.items.some(i=>i.sizeId===id))p.items.push({sizeId:id,price:'',share:p.items.length?0:100,weight:''});commit();close(true);return;}
    ...
    if(kind==='shop'){
      ...
      if(modal.id)state.shops.find(x=>x.id===modal.id).name=d.name;else{const s={id:M.uid('shop'),name:d.name,deleted:false};state.shops.push(s);state.activeShop=s.id;state.active='';view='plans';}
    }
    if(kind==='plan'){
      ...
      if(modal.id)state.plans.find(x=>x.id===modal.id).name=d.name;else{const created={...M.newPlan(state,state.activeShop,d.name),materialId:d.materialId,shippingId:d.shippingId};state.plans.push(created);state.active=created.id;view='plan';tab='sku';}
    }
    if(kind==='material'){
      ...
      const rules=(d.weightRules||[]).map((r,i)=>({...r,id:r.id||M.uid('rule'),thickness:r.thickness===''||r.thickness===undefined?'':Number(r.thickness),variant:r.variant||'',coefficient:r.coefficient===''?'':Number(r.coefficient),costPerSqm:r.costPerSqm===''?'':Number(r.costPerSqm),default:!!r.default}));
      ...
      if(new Set(rules.map(r=>`${r.thickness}|${r.variant}`)).size!==rules.length)return fail('同一材料的厚度与变体组合不能重复');
      if(!rules.some(r=>r.default))rules[0].default=true;
      let m=state.materials.find(x=>x.id===modal.id);if(!m){const price=rules.find(r=>r.default)?.costPerSqm??rules[0].costPerSqm;m={id:M.uid('material'),name:d.name,price,description:'',active:true,history:[]};state.materials.push(m);}
      const current=rules.find(r=>r.default)||rules[0];
      ...
      Object.assign(m,{name:d.name,price:current.costPerSqm,weightRules:rules});
    }
    if(kind==='size'){
      const value={id:modal.id||M.uid('size'),name:d.name.trim(),salesW:d.salesW,salesH:d.salesH,irregular:d.irregular,productionW:d.irregular?d.productionW:'',productionH:d.irregular?d.productionH:'',active:d.active??true,needsReview:false};
    ...
    if(kind==='shipping'){
      const t={...M.clone(d),id:modal.id||M.uid('shipping'),active:d.active??true};
    ...
    if(kind==='weight'){const template=state.shippingTemplates.find(t=>t.id===p.shippingId);if(d.weight!==''&&(!M.positive(d.weight)||d.weight>1000||M.shippingCost(template,d.weight).error))return fail('请填写模板支持范围内大于 0 的含包装重量，或留空恢复自动计算。','[data-field="weight"]');p.items.find(i=>i.sizeId===modal.id).weight=d.weight;}
  ...
  document.addEventListener('click',async event=>{
    ...
    try{
      ...
      if(a==='save-modal')return saveModal();
      ...
      if(a==='product-view'){view='product';render();return;}
      ...
      if(a==='plan-view'){view='plan';render();return;}
      if(a==='plans-view'){view='plans';render();return;}
      if(a==='shops'){view='shops';render();return;}
      if(a==='ledger-view'){view='history';render();return;}
      if(a==='data-view'){view='data';render();return;}
      if(a==='library-view'||a==='shipping-library'){view='library';if(a==='shipping-library')libraryTab='shipping';render();return;}
      ...
      if(a==='select-plan'){state.active=id;state.activeShop=state.plans.find(x=>x.id===id).shopId;view='plan';tab='sku';commit();return;}
      if(a==='select-shop'){state.activeShop=id;normalize();view='plan';commit();return;}
      ...
      if(a==='copy-plan'){const copied=M.clone(p);copied.id=M.uid('plan');copied.name=p.name+' · 副本';state.plans.push(copied);state.active=copied.id;commit();toast('已复制试算参数，历史账目不复制');return;}
      ...
      if(a==='add-material-rule'){modal.draft.weightRules=[...(modal.draft.weightRules||[]),{id:M.uid('rule'),thickness:'',variant:'',coefficient:'',costPerSqm:'',default:false}];paintModal();return;}
      if(a==='remove-material-rule'){modal.draft.weightRules.splice(Number(b.dataset.index),1);paintModal();return;}
      ...
      if(a==='add-skus')return open('add-skus',{ids:p.items.map(i=>i.sizeId)});
      if(a==='remove-sku')return confirmAction('移除这个商品规格？','将移除本计划中的售价和占比；公共尺寸及历史账目保留。',()=>{p.items=p.items.filter(i=>i.sizeId!==id);commit();},'移除');
      if(a==='weight')return open('weight',{id,draft:{weight:p.items.find(i=>i.sizeId===id).weight}});
      ...
      if(a==='confirm-restore'){
        ...
        const replacing=modal.restoreMode==='replace'&&!modal.data.exportScope,merged=modal.committed||replacing?null:T.merge(state,modal.data,{restorePlans:!!modal.restorePlans}),next=modal.committed?state:replacing?M.clone(modal.data):merged.state;
        if(!M.validateBackup(next))return fail('恢复数据校验未通过');delete next.exportScope;
        const skipped=merged?.report.conflicts.length||0;view='plan';tab='sku';
        await persistModal(next,replacing?'restore':'save',skipped?`合并完成；${skipped} 个冲突计划的账目保留本机版本`:'工作区已恢复并保存，重复账目不会累计');return;
      }
  ...
  document.addEventListener('input',event=>{
    ...
    if(el.matches('[data-param]')){p.params[el.dataset.param]=value;save();refreshCalculation();}
    if(el.matches('[data-param-rate]')){p.params.refundRates={...(p.params.refundRates||{}),[el.dataset.paramRate]:value};save();refreshCalculation();}
    ...
    if(el.matches('[data-item]')){p.items.find(i=>i.sizeId===el.dataset.item)[el.dataset.key]=value;save();refreshCalculation();}
    ...
    if(modal&&el.matches('[data-entry-price]')){M.setFrameMaterialPrice(modal.frame,value);document.querySelector('[data-quote]').value='';refreshEntry();}
    if(modal&&el.matches('[data-entry-item]')){modal.frame.plan.items.find(i=>i.sizeId===el.dataset.entryItem)[el.dataset.key]=value;refreshEntry();}
  });
  document.addEventListener('change',event=>{
    const el=event.target,p=current();if(operationPending||modal?.committed&&dialog.contains?.(el))return;const value=readInput(el);
    if(el.id==='active-shop'){state.activeShop=value;normalize();view='plan';commit();}
    if(el.matches('[data-product-file]')){const file=el.files?.[0];if(file)analyzeProductFiles(file);return;}
    if(el.matches('[data-plan-material]')){p.materialId=value;p.materialRuleId=M.materialRule(state.materials.find(m=>m.id===value),{} )?.id;save();render();}
    if(el.matches('[data-plan-material-rule]')){p.materialRuleId=value;save();refreshCalculation();render();}
    if(el.matches('[data-plan-shipping]')){p.shippingId=value;save();render();}
    if(el.matches('[data-param-scope]')){p.params.otherFeeScope=value;save();refreshCalculation();}
  ...
  document.querySelector('#backup-input').addEventListener('change',async event=>{
    ...
    toast('正在检查备份…');try{const data=file.name.toLowerCase().endsWith('.json')?M.migrate(JSON.parse(await file.text())):await W.importWorkbook(await file.arrayBuffer());if(M.duplicateMaterialName(data.materials||[]))return toast('备份包含同名材料，请在来源工作台为材料设置不同名称后重新导出。');if(!M.validateBackup(data))throw Error('备份未通过检查，当前数据保留');open('restore',{data});}catch(error){toast(safeError(error,'文件无法恢复，请选择未修改的完整备份，并检查材料是否有同名项。'));}
  });
  window.addEventListener('beforeunload',event=>{if(queue?.pending){queue.remember();event.preventDefault();event.returnValue='';}});
