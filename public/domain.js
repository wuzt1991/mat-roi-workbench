...
(function(root) {
  ...
  // Legacy shape fields are read only for compatibility. Every current UI uses this one pair.
  const productionDimensions=s=>({width:s?.irregular?s.productionW:s?.salesW,height:s?.irregular?s.productionH:s?.salesH});
  const productionSizeLabel=s=>s?.name||`${productionDimensions(s).width||'—'} × ${productionDimensions(s).height||'—'} cm`;
  function productionArea(s) {
    if(!s||s.needsReview)return NaN;
    const {width:w,height:h}=productionDimensions(s);
  ...
  function calculate(state,plan,overrides={}) {
    ...
    const rows=plan.items.map((original,index)=>{
      ...
      if(!validSize(size)||!Number.isFinite(area))errors.push('请补齐规格的实际生产尺寸');
  ...
  function confirmRecord(state,{frame,date,note='',previousId=null,reason=''}) {
    ...
    if(!r.valid||r.profit===null||r.gmv===null)throw Error(r.errors[0]||'请补齐当天广告消耗和成交金额或支付 ROI');
  ...
  const recordResultFields=['price','cost','margin','netMargin','roi','netRoi','gmv','profit','investment','orders','revenue','netRevenue','goods','shipping','fees','tax','other'];
  const legacyOptionalResultFields=new Set(['netMargin','netRoi','netRevenue']);
  function recordResultKeys(h){
    // Early v3 snapshots did not store net metrics. Keep their frozen JSON intact;
    // validate every metric they did store. Versioned v4 snapshots require all fields.
    return recordResultFields.filter(k=>h.frame?.calculationVersion!==undefined||!legacyOptionalResultFields.has(k)||Object.prototype.hasOwnProperty.call(h.result||{},k));
  }
  function validRecordResults(s){return s.records.every(h=>!h.frame||recordResultKeys(h).every(k=>Object.prototype.hasOwnProperty.call(h.result||{},k)));}
