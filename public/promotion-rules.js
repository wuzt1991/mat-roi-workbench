...
(function(root){
  ...
  function forward(listingPrice,scheme){
    ...
    if(!scheme)return {listingPrice,finalPrice:listingPrice,steps:[]};
  ...
  function reverse(targetPrice,scheme){
    ...
    if(!scheme)return {listingPrice:targetPrice,finalPrice:targetPrice,steps:[],exact:true,difference:0};
    const invalid=errors(scheme);if(invalid.length)return {error:invalid.join('；')};
  ...
  if(typeof module==='object')module.exports=api;else root.PromotionRules=api;
