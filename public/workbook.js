(function(root){
  ...
  function tables(s,format=3){
    ...
    const sheets=[
      ...
      ['运费模板',[['模板','类型','重量下限（不含，kg）','重量上限（含，kg）','固定或分档费用（元）','首重（kg）','首重费用（元）','续重（kg）','每续重费用（元）','计费说明','状态'],...s.shippingTemplates.flatMap(t=>t.type==='tiers'?t.tiers.map((v,i)=>[t.name,'重量分档',i?t.tiers[i-1].upTo:0,v.upTo,v.fee,'','','','','按重量所在档收取整单运费',t.active?'启用':'停用']):[[t.name,t.type==='fixed'?'固定运费':'首重续重',t.type==='step'?0:'',t.maxWeight,t.fee,t.firstWeight,t.firstFee,t.stepWeight,t.stepFee,t.type==='step'?'不足一个续重按一个计费':'每单固定收取',t.active?'启用':'停用']])]],
      ['历史账目',[['记录编号','日期','店铺','计划','状态','材料','当时单价（元/㎡）','广告费（元）','支付销售额（元）','保本ROI','预估盈亏（元）','入账备注','更正原因','原记录编号'],...s.records.map(h=>[h.id,h.date,h.shopName,h.planName,status(h),h.frame?.materials[0].name||h.legacy?.materialName,h.frame?.materials[0].price??h.legacy?.materialPrice,h.frame?.plan.params.spend??h.legacy?.spend,h.result.gmv,M.ceilRoi(h.result.roi),h.result.profit,h.note,h.reason,h.previousId])]],
  ...
  async function exportWorkbook(state,scope){
    ...
    const data=book.addWorksheet('恢复数据');data.state='veryHidden';data.addRow(['MAT-ROI-XLSX',3]);
  ...
  async function importWorkbook(bytes){
    ...
    if(!sheet||sheet.getCell('A1').value!=='MAT-ROI-XLSX'||![1,2,3].includes(sheet.getCell('B1').value))throw Error('请选择由工作台导出的完整或按范围 Excel 备份');
  ...
  const api={tables,exportWorkbook,importWorkbook,exportLedgerWorkbook};if(typeof module==='object')module.exports=api;else root.MatWorkbook=api;
