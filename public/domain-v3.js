...
(function(root) {
  ...
  const metricList=[{id:'roi',label:'保本 ROI'},{id:'netRoi',label:'净 ROI'},{id:'profit',label:'预估盈亏'},{id:'cost',label:'平均每单成本'},{id:'price',label:'平均售价'},{id:'margin',label:'每单可投广告费'},{id:'gmv',label:'支付销售额'},{id:'investment',label:'总投入'},{id:'rate',label:'每百元结余'}];
  const skuColumnList=[{id:'price',label:'售价',unit:'元'},{id:'share',label:'订单占比',unit:'%'},{id:'weight',label:'发货重量',unit:'g'},{id:'material',label:'材料成本'},{id:'shipping',label:'运费'},{id:'cost',label:'每单总成本'},{id:'roi',label:'保本 ROI'}];
  ...
  function normalizeMaterial(material){
    const m=clone(material||{});const parseThickness=v=>{if(v===''||v===undefined||v===null)return '';if(typeof v==='number')return v;const n=Number(String(v).replace(/毫米|mm/ig,'').trim());return Number.isFinite(n)?n:v;};m.weightRules=Array.isArray(m.weightRules)&&m.weightRules.length?m.weightRules.map((r,i)=>({...r,id:/^[a-zA-Z0-9_-]{1,100}$/.test(r.id||'')?r.id:ruleId(m.name,r.thickness,r.variant)||`rule-${m.id}-${i}`,thickness:parseThickness(r.thickness),variant:r.variant??'',coefficient:Number(r.coefficient??r.weightPerSqm),costPerSqm:r.costPerSqm===undefined||r.costPerSqm===''?Number(m.price):Number(r.costPerSqm),default:!!r.default})):builtinRules(m.name);
    // Custom materials without explicit rules remain unresolved until configured in the material library.
    ...
    if(!m.weightRules.some(r=>r.default)&&m.weightRules[0])m.weightRules[0].default=true;
    return m;
  }
  ...
  function materialRule(material,plan){const all=material?.weightRules||[],rules=all.filter(r=>!r.variant);return rules.find(r=>r.id===plan?.materialRuleId)||rules.find(r=>r.default)||rules[0]||all[0]||null;}
  ...
  function finalizeState(s){
    ...
    for(const h of (s.records||[])) if(h.frame){
      ...
      if(fp){fp.params=normalizeParams(fp.params);if(fr?.id)fp.materialRuleId=fr.id;else delete fp.materialRuleId;}
    }
    return s;
  }
  ...
  function calculate(state,plan,overrides={}) {
    ...
    const rows=plan.items.map(item=>{
      ...
      return {...item,weight,size,area,material,baseShipping,shipping,revenue,netRevenue,goods,fees,tax,other,otherBase,returnExtra,refundTotal,unshipped,shippedOnly,returnRefund,firstHour,cost,margin,netMargin,roi:margin>0?item.price/margin:null,netRoi:netMargin>0?item.price/netMargin:null};
    });
  ...
  function confirmRecord(state,{frame,date,note='',previousId=null,reason=''}) {
    ...
    const p=frame.plan,r=calculate(frame,p),previous=state.records.find(h=>h.id===previousId);
  ...
  function newPlan(state,shopId,name){const material=state.materials.find(m=>m.active&&(m.weightRules||[]).some(usableMaterialRule))||state.materials.find(m=>m.active)||state.materials[0];return {id:uid('plan'),shopId,name,note:'',materialId:material?.id||'',shippingId:state.shippingTemplates.find(t=>t.active)?.id||state.shippingTemplates[0].id,params:{...defaults,refundRates:{...defaultRefundRates},otherFeeScope:'shipped'},items:[],deleted:false};}
  function migrate(input) {
    if(input?.version===3)return finalizeState(clone(input));
    const old=Legacy.migrate(input);
    if(!Legacy.validateBackup(old))throw Error('旧备份未通过检查，原数据保留');
    ...
    old.plans.forEach(p=>{
      const fee=Number(p.params.shipping),existing=s.shippingTemplates.find(t=>t.fee===fee),template=existing||{id:uid('shipping'),name:`固定运费 ${fee} 元`,type:'fixed',fee,active:true};
      if(!existing)s.shippingTemplates.push(template);
    ...
    });return finalizeState(s);
  }
  function initialState(){
    const shop={id:uid('shop'),name:'我的店铺',deleted:false},material={id:uid('material'),name:'我的地垫材料',price:10.2,description:'',active:true,history:[{id:uid('quote'),date:today(),price:10.2,note:'初始报价，可修改'}]};
    const s={version:3,shops:[shop],activeShop:shop.id,active:'',materials:[material],sizes:[[30,40],[40,60],[50,80],[60,90],[80,100],[80,120]].map(([salesW,salesH])=>({id:uid('size'),name:'',salesW,salesH,irregular:false,productionW:'',productionH:'',active:true})),shippingTemplates:[regionalShippingTemplate()],plans:[],records:[],prefs:{ids:['roi','profit']}};
    s.plans.push(newPlan(s,shop.id,'我的第一个计划'));s.active=s.plans[0].id;return finalizeState(s);
  }
  function seed() {
    const s=migrate(Legacy.seed());s.shops[0].name='示例店铺一';
    const second={id:'shop-second',name:'示例店铺二',deleted:false};s.shops.push(second);s.plans[1].shopId=second.id;
    s.records.forEach(h=>{h.shopId=s.plans.find(p=>p.id===h.planId).shopId;h.shopName=s.shops.find(x=>x.id===h.shopId).name;});
    s.shippingTemplates.push({id:'shipping-tier',name:'重量分档（示例）',type:'tiers',tiers:[{upTo:.5,fee:1.35},{upTo:1,fee:1.8},{upTo:2,fee:2.8}],active:true},{id:'shipping-step',name:'首重续重（示例）',type:'step',firstWeight:1,firstFee:2,stepWeight:.5,stepFee:.8,maxWeight:10,active:true});return s;
  }
  function validateBackup(s) {
    try {
      const id=v=>typeof v==='string'&&/^[a-zA-Z0-9_-]{1,100}$/.test(v),text=v=>typeof v==='string'&&v.length<=10000,unique=a=>Array.isArray(a)&&a.length<=20000&&a.every(x=>x&&id(x.id))&&new Set(a.map(x=>x.id)).size===a.length;
      const validRefundParams=params=>{
        if(!params||params.refundRates===undefined)return true;
        const rates=params.refundRates;if(!rates||typeof rates!=='object'||Array.isArray(rates))return false;
        for(const key of ['unshipped','shippedOnly','returnRefund','firstHour'])if(!draft(rates[key])||rates[key]!==''&&rates[key]>100)return false;
        const values=['unshipped','shippedOnly','returnRefund'].map(key=>Number(rates[key]));
        const total=values.every(Number.isFinite)?values.reduce((a,v)=>a+v,0):NaN,first=rates.firstHour===''?'':Number(rates.firstHour);
      ...
      for(const h of s.records){
        ...
        if(h.kind==='daily'){
          if(!Number.isFinite(h.result.profit)||!nonnegative(h.result.gmv))return false;
          if(h.frame){const p=h.frame.plan;if(p.id!==h.planId||p.shopId!==h.shopId||!validRefundParams(p.params))return false;const r=calculate(h.frame,p);if(!r.valid||Object.keys(h.result).some(k=>Object.prototype.hasOwnProperty.call(summarize(r),k)&&!near(r[k],h.result[k])))return false;}
          else if(h.legacy){
            const l=h.legacy,mat={id:'frozen',name:l.materialName,price:l.materialPrice,baseThickness:l.materialBaseThickness,active:true,history:[]};
            const frozen={version:2,materials:[mat],sizes:l.items.map(i=>({...i,id:i.sizeId,active:true})),plans:[{id:'frozen-plan',name:'历史校验',materialId:mat.id,params:l.params,items:l.items,history:[l]}]};
            if(!Legacy.validateBackup(frozen)||!['profit','gmv','roi','price'].every(k=>near(l[k],h.result[k]))||l.cost!==undefined&&!near(l.cost,h.result.cost))return false;
          }
          else return false;
          if(h.status==='confirmed'){const key=h.planId+'/'+h.date;if(dates.has(key))return false;dates.add(key);}
        }
        if(h.previousId&&!s.records.some(x=>x.id!==h.id&&x.id===h.previousId&&x.planId===h.planId&&x.kind===h.kind&&x.status==='superseded'&&x.replacedBy===h.id))return false;
        if(h.status==='superseded'&&!s.records.some(x=>x.id!==h.id&&x.id===h.replacedBy&&x.previousId===h.id))return false;
        if(h.status!=='superseded'&&h.replacedBy)return false;
      }
      const byId=new Map(s.records.map(h=>[h.id,h])),visited=new Set();
      for(const h of s.records){const chain=new Set();let node=h;while(node&&!visited.has(node.id)){if(chain.has(node.id))return false;chain.add(node.id);node=byId.get(node.previousId);}for(const id of chain)visited.add(id);}
      if(s.prefs?.skuColumns!==undefined&&(!Array.isArray(s.prefs.skuColumns)||s.prefs.skuColumns.length>skuColumnList.length||new Set(s.prefs.skuColumns).size!==s.prefs.skuColumns.length||!s.prefs.skuColumns.every(id=>skuColumnList.some(x=>x.id===id))))return false;
  ...
  if(typeof module==='object')module.exports=api;else root.MatModelV3=api;
