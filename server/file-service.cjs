...
function createFileService({store,dataDir}){
  ...
  function openSession(id){return new ImportSessionStore(sessionDirectory(id));}
  function phaseFor(type){return type==='inspect'?'inspecting':type==='import'||type==='aggregate-sales'?'importing':type==='apply-review'?'applying':'exporting';}
  ...
  function onJobChange(job){
    if(!job.sessionId||job.state==='running')return;let session;try{session=openSession(job.sessionId);}catch{return;}try{session.setMeta('job',job);if(job.state==='succeeded'){
      ...
      else if(['import','aggregate-sales','apply-review'].includes(job.type))session.updateMeta({phase:'reviewing',error:null});
  ...
  function startSessionJob(id,type,payload={},extra={}){
    if(restoring)throw new SessionError(409,'正在恢复工作区，文件任务暂停。','RESTORE_IN_PROGRESS');const directory=sessionDirectory(id),session=openSession(id),meta=session.metadata();session.close();const sourcePath=path.join(directory,'source.xlsx'),outputDir=path.join(directory,'artifacts');fs.mkdirSync(outputDir,{recursive:true});const outputPath=path.join(outputDir,`${crypto.randomUUID()}.${type==='export-rescue'?'ndjson':'xlsx'}`),job=broker.start(type,payload,{sessionId:id,workspaceId:meta.workspaceId,storageEpoch:meta.storageEpoch,generation:meta.generation,sessionDirectory:directory,sourcePath,templatePath:TEMPLATE,outputPath,outputDir,...extra}),marker=openSession(id);try{marker.updateMeta({job,phase:phaseFor(type),error:null});}finally{marker.close();}return job;
  }
  ...
  async function handle(request,response,url){
    ...
    if(parts[3]==='sales-reviews'&&request.method==='POST'){const input=await readJson(request),session=openSession(id);try{if(session.getMeta('kind')!=='sales')throw new SessionError(422,'该会话不是销售导入。','INVALID_SESSION_KIND');return json(response,200,session.salesReview(input)),true;}finally{session.close();}}
    if(parts[3]==='reviews'&&request.method==='POST'){const input=await readJson(request),session=openSession(id);try{const meta=session.assertContext(input),rules=currentRules();if(digest(rules)!==meta.rulesFingerprint)throw new SessionError(409,'可复用规则已变更，请重新计算后复核。','RULES_CHANGED');const targets=session.targetRowIds(input);if(targets.length>1000){session.close();const job=startSessionJob(id,'apply-review',{...input,rules});return json(response,202,{jobId:job.jobId}),true;}const receipt=session.applyReview(input,rules);return json(response,200,receipt),true;}finally{try{session.close();}catch{}}}
    if(parts[3]==='undo'&&request.method==='POST'){const input=await readJson(request),session=openSession(id);try{return json(response,200,session.undo(input,currentRules())),true;}finally{session.close();}}
    if(parts[3]==='export'&&request.method==='POST'){const input=await readJson(request),session=openSession(id);try{const meta=session.assertContext(input);if(digest(currentRules())!==meta.rulesFingerprint)throw new SessionError(409,'规则已变更，请先重新计算。','RULES_CHANGED');if(!session.counts(meta.generation).ready)throw new SessionError(422,'仍有未确认规格，禁止导出。','SESSION_NOT_READY');const job=startSessionJob(id,'export-product',{});return json(response,202,{jobId:job.jobId}),true;}finally{session.close();}}
  ...
  async function withWorkspaceWrite(fn,state){if(workspaceWrite)throw new SessionError(409,'工作区正在提交关联规则。','WORKSPACE_WRITE_BUSY');workspaceWrite=true;try{return await fn();}finally{workspaceWrite=false;}}
