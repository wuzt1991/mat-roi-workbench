...
function createFileService({store,dataDir}){
  ...
  function assertRules(meta){const rules=currentRules();if(digest(rules)!==meta.rulesFingerprint||(meta.kind==='product'&&meta.derivationVersion!==Recognition.DERIVATION_VERSION))throw new SessionError(409,'可复用规则已变更，请重新计算后复核。','RULES_CHANGED');return rules;}
  ...
  async function handle(request,response,url){
    ...
    if(parts[3]==='select-sheet'&&request.method==='POST'){
      ...
      try{
        ...
        const payload=sales?{...selections[0],rules,period:input.period||null,matchBy:input.matchBy,basis:input.basis}:{selections,rules,requireThicknessSetup:input.requireThicknessSetup===true,thicknessDefaults:Recognition.normalizeThicknessDefaults(input.thicknessDefaults||{},rules)};
        const job=startSessionJob(id,type,payload);
        return json(response,202,{jobId:job.jobId}),true;
      }finally{session.close();}
    }
    if(parts[3]==='rows'&&request.method==='GET'){const session=openSession(id);try{const meta=assertCurrent(session.metadata()),rulesStale=(digest(currentRules())!==meta.rulesFingerprint||meta.derivationVersion!==Recognition.DERIVATION_VERSION),page=session.page({view:url.searchParams.get('view')||'rows',groupId:url.searchParams.get('groupId')||'',materialPage:url.searchParams.get('materialPage')||1,search:url.searchParams.get('search')||'',attention:url.searchParams.get('attention')==='1',status:url.searchParams.get('status')||'all',missingThickness:url.searchParams.get('missingThickness')==='1',page:url.searchParams.get('page')||1,pageSize:url.searchParams.get('pageSize')||100});return json(response,200,{...page,rulesStale,ready:page.ready&&!rulesStale&&(!meta.requireThicknessSetup||meta.uniformConfirmed===true)}),true;}finally{session.close();}}
