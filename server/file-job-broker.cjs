...
class FileJobBroker{
  ...
  start(type,payload,context={}){
    ...
    // Exit can precede the final IPC message on Windows. Drain the channel first.
    child.once('close',(code,signal)=>{if(!settled)finish(job.cancelRequested?'canceled':'failed',null,{code:job.cancelRequested?'CANCELED':'CHILD_EXIT',message:job.cancelRequested?'任务已取消':`文件子进程异常退出 (${code??signal})`});if(this.active===job)this.active=null;});
