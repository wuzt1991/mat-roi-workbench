...
class FileJobBroker{
  ...
  start(type,payload,context={}){
    ...
    const env={...process.env,ELECTRON_RUN_AS_NODE:'1'},child=fork(this.entry,[],{env,stdio:['ignore','ignore','ignore','ipc']});job.child=child;let settled=false,completion=null;
    ...
    const finish=(state,result,error)=>{if(settled||completion)return;completion={state,result:result||null,error:error||null};for(const timer of Object.values(job.timers))clearTimeout(timer);if(!child.killed)child.kill();};
    child.on('message',message=>{if(message?.jobId!==jobId||settled||completion)return;if(message.type==='progress'){job.progress=message.progress||{};touch();}else if(message.type==='result')finish('succeeded',message.result,null);else if(message.type==='error')finish(message.error?.code==='CANCELED'?'canceled':'failed',null,message.error);});
    child.once('error',error=>finish('failed',null,{code:error.code||'CHILD_ERROR',message:error.message}));
    // Drain IPC and release the worker before publishing completion to the next job.
    child.once('close',(code,signal)=>{if(settled)return;settled=true;for(const timer of Object.values(job.timers))clearTimeout(timer);if(this.active===job)this.active=null;Object.assign(job,completion||{state:job.cancelRequested?'canceled':'failed',result:null,error:{code:job.cancelRequested?'CANCELED':'CHILD_EXIT',message:job.cancelRequested?'任务已取消':`文件子进程异常退出 (${code??signal})`}});job.updatedAt=new Date().toISOString();this.onChange?.(this.snapshot(job));});
