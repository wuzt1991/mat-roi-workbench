...
class FileJobBroker{
  ...
  start(type,payload,context={}){
    ...
    const finish=(state,result,error)=>{if(settled)return;settled=true;for(const timer of Object.values(job.timers))clearTimeout(timer);job.state=state;job.result=result||null;job.error=error||null;job.updatedAt=new Date().toISOString();if(this.active===job)this.active=null;this.onChange?.(this.snapshot(job));if(child.connected)child.disconnect();if(!child.killed)child.kill();};
    ...
    child.once('error',error=>finish('failed',null,{code:error.code||'CHILD_ERROR',message:error.message}));child.once('exit',(code,signal)=>{if(!settled)finish(job.cancelRequested?'canceled':'failed',null,{code:job.cancelRequested?'CANCELED':'CHILD_EXIT',message:job.cancelRequested?'任务已取消':`文件子进程异常退出 (${code??signal})`});});
  ...
  async stop(){this.closed=true;if(this.active){this.cancel(this.active.jobId);await new Promise(resolve=>{const child=this.active?.child;if(!child)return resolve();const timer=setTimeout(resolve,3200);child.once('exit',()=>{clearTimeout(timer);resolve();});});}}
}
